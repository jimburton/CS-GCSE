...
import sys
# to do:
# background music / pygame.mixer.music.load(insert music) and stop it by using/ pygame.mixer.music.stop()
#add point amount / victory screen by:
# keep track of players score / when reaching required score, go to victory screen and ask if you want to play again
# fix speed increase
...
def loading_screen():
    ...
    my_font2 = pygame.font.SysFont("ComicSansMS",25)
    global in_play
    in_play = False
    while True:
...
win2_text = my_font.render(f"player 2 wins!!!",True,RED)
win_text = my_font.render(f"player 1 wins!!!",True,GREEN)
win_width = win_text.get_rect().width/2
win_height = win_text.get_rect().height/2
win2_width = win2_text.get_rect().width/2
win2_height = win2_text.get_rect().height/2
def play_game():
    global in_play
    while True:
        for event in pygame.event.get():               
        ...
        if player_score == 3:
            while player_score == 3:
                in_play = False
                DISPLAYSURF.blit(win_text,(SCREEN_WIDTH/2 - win_width,SCREEN_HEIGHT/2 - win_height))
        elif computer_score == 3:
            while computer_score == 3:
                in_play = False
                DISPLAYSURF.blit(win2_text,(SCREEN_WIDTH/2 - win2_width,SCREEN_HEIGHT/2 - win2_height))   
